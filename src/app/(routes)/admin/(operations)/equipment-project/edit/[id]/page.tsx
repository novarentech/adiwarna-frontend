...
import { MdEdit } from "react-icons/md"; // Mengganti ikon menjadi MdEdit
...
import {
    ...
    updateEquipmentproject, // Fungsi untuk UPDATE
    getEquipmentprojectById, // Fungsi untuk GET BY ID
    EquipmentItem,
    CreateEquipmentProjectPayload, // Payload sama untuk create/update
...
// Definisikan props untuk menerima ID proyek
type EditEquipmentProjectPageProps = Promise<{ id: string }>;


export default function EditEquipmentProjectPage({ params }: { params: EditEquipmentProjectPageProps }) {
    ...
    const [isProjectLoading, setIsProjectLoading] = useState(true); // Loading untuk data awal
    ...
    const handleFormChange = (
        ...
        if (id === "customer_id") {
            ...
            setCustomersLocation([]); // Reset lokasi agar fetch ulang
        }
    };


    // --- 2. FETCH DATA CUSTOMER (Dropdown) ---
    useEffect(() => {
    ...
    // --- 3. FETCH DATA PROJECT UNTUK EDIT (HYDRATION) ---
    useEffect(() => {
        const fetchProjectDetail = async () => {
            if (!projectId) return;

            setIsProjectLoading(true);
            const result = await getEquipmentprojectById(Number(projectId)) as GetEquipmentProjectByIdResponse;

            if (result.success && result.data) {
                const project = result.data;

                // ASUMSI: API mengembalikan ID customer/lokasi. Jika tidak, logika ini perlu diubah
                // atau API harus diubah.
                const initialCustomerId = String(project.customer_id);
                const initialLocationId = String(project.customer_location_id);

                setFormData({
                    project_date: project.project_date,
                    customer_id: initialCustomerId,
                    customer_location_id: initialLocationId,
                    prepared_by: project.prepared_by,
                    verified_by: project.verified_by,
                });

                // Set peralatan yang sudah terpilih
                const initialSelectedEquipment = project.equipments.map(e => e.id);
    ...
    // --- 4. FETCH CUSTOMER LOCATIONS (berdasarkan formData.customer_id) ---
    useEffect(() => {
        ...
        // Hanya fetch jika customer_id sudah terisi, atau ketika loading proyek selesai
        if (formData.customer_id || !isProjectLoading) {
            fetchCustomerDetail();
        }
    }, [formData.customer_id, isProjectLoading]); // Tambahkan isProjectLoading untuk sinkronisasi


    // --- 5. FETCH DATA EQUIPMENT TERSEDIA (Tabel) ---
    ...
    const handleSelectAllEquipment = () => {
        const allIdsOnPage = availableEquipment.map(item => item.id);

        // Cek berapa banyak item di halaman saat ini yang sudah terpilih
        const selectedOnPage = allIdsOnPage.filter(id => selectedEquipmentIds.includes(id));

        if (selectedOnPage.length === allIdsOnPage.length) {
            // Jika semua di halaman saat ini terpilih, hapus semua dari halaman ini
            setSelectedEquipmentIds(prev => prev.filter(id => !allIdsOnPage.includes(id)));
        } else {
            // Jika tidak semua atau tidak ada yang terpilih, tambahkan semua dari halaman ini
            const newSelected = new Set([...selectedEquipmentIds, ...allIdsOnPage]);
            setSelectedEquipmentIds(Array.from(newSelected));
        }
    };

    // Helper: Apakah semua item di halaman saat ini terpilih?
    const isAllSelectedOnPage = availableEquipment.every(item => selectedEquipmentIds.includes(item.id));
    const isIndeterminate = availableEquipment.some(item => selectedEquipmentIds.includes(item.id)) && !isAllSelectedOnPage;


    // --- 7. HANDLE UPDATE PROJECT ---
    const handleUpdateProject = async (e: React.FormEvent) => {
        ...
        console.log(`Update Project ID ${projectId} Payload:`, payload);
    ...
    if (isProjectLoading) {
        return (
            <div className="w-full h-full px-4 py-4 bg-[#f4f6f9] flex justify-center items-center">
                <p className="text-xl font-medium">Loading Project Data...</p>
            </div>
        );
    }
    ...
    return (
        <div className="w-full h-full px-4 py-4 bg-[#f4f6f9]">
            ...
            <div className="flex flex-row items-center space-x-2 mt-2">
                ...
                <h1 className="text-3xl font-normal">Edit Equipment Project - ID: {projectId}</h1>
            </div>

            {/* start of form container */}
            <div className="bg-white border rounded-sm px-5 py-6 shadow-xs my-12 ">
                {/* start form */}
                <form onSubmit={handleUpdateProject} className="flex flex-col">
                    ...
                    <div className="mt-4">
                        <h2 className="text-xl font-bold mb-4">Pilih Equipment Tersedia (ID Terpilih: {selectedEquipmentIds.length})</h2>
                                    ...
                                    <TableRow className="bg-[#f2f2f2] hover:bg-[#f2f2f2]">
                                        <TableHead className="w-12 text-center">
                                            <input type="checkbox"
                                                ...
                                                ref={el => {
                                                    // Jika el ada, jalankan assignment, lalu pastikan return type-nya void/undefined
                                                    el && (el.indeterminate = isIndeterminate), undefined;
                                                }}
                                            />
                                        </TableHead>
