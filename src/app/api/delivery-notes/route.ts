...
import { GetAllDeliveryNoteResponse } from "@/lib/delivery-notes";
...
export async function GET(req: Request) {
    try {
        ...
        const res = await fetch(`${apiBaseUrl}/delivery-notes?per_page=${perPage}&page=${page}&search=${search}`, {
            headers: {
        ...
        // Parse response JSON
        const data: GetAllDeliveryNoteResponse = await res.json();
...
export async function POST(req: Request) {
    try {
        ...
        const res = await fetch(`${apiBaseUrl}/delivery-notes`, {
            method: "POST",
            headers: {
